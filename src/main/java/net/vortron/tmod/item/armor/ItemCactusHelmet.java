...
public class ItemCactusHelmet extends ItemArmor {
    public ItemCactusHelmet() {
        super(Materials.cactusArmorMaterial,0, EntityEquipmentSlot.HEAD);
        this.setCreativeTab(CreativeTab.TMod);
        ItemUtil.nameItemId(this, "cactus_helmet");
    }
    public static final Item item = new ItemCactusHelmet();


    @SubscribeEvent
    public static void registerItems(RegistryEvent.Register<Item> event) {
        event.getRegistry().register(ItemCactusHelmet.item);
    }
    ...
    public static void prickPlayers(LivingDamageEvent event) {
        if (event.getEntity() instanceof EntityPlayer) {
            ...
            if(player.getItemStackFromSlot(EntityEquipmentSlot.HEAD).isItemEqualIgnoreDurability(item.getDefaultInstance())) {
