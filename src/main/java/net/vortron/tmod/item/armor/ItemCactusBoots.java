...
public class ItemCactusBoots extends ItemArmor {
    public ItemCactusBoots() {
        super(Materials.cactusArmorMaterial,0, EntityEquipmentSlot.FEET);
        this.setCreativeTab(CreativeTab.TMod);
        ItemUtil.nameItemId(this, "cactus_boots");
    }
    public static final Item item = new ItemCactusBoots();


    @SubscribeEvent
    public static void registerItems(RegistryEvent.Register<Item> event) {
        event.getRegistry().register(ItemCactusBoots.item);
    }
    ...
    public static void prickPlayers(LivingDamageEvent event) {
        if (event.getEntity() instanceof EntityPlayer) {
            ...
            if(player.getItemStackFromSlot(EntityEquipmentSlot.FEET).isItemEqualIgnoreDurability(item.getDefaultInstance())) {
