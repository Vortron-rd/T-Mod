...
public class ItemCactusLeggings extends ItemArmor {
    public ItemCactusLeggings() {
        super(Materials.cactusArmorMaterial,0, EntityEquipmentSlot.LEGS);
        this.setCreativeTab(CreativeTab.TMod);
        ItemUtil.nameItemId(this, "cactus_leggings");
    }
    public static final Item item = new ItemCactusLeggings();


    @SubscribeEvent
    public static void registerItems(RegistryEvent.Register<Item> event) {
        event.getRegistry().register(ItemCactusLeggings.item);
    }
    ...
    public static void prickPlayers(LivingDamageEvent event) {
        if (event.getEntity() instanceof EntityPlayer) {
            ...
            if(player.getItemStackFromSlot(EntityEquipmentSlot.LEGS).isItemEqualIgnoreDurability(item.getDefaultInstance())) {
