...
public class ItemCompressedDiamondCharm extends Item implements IBauble {
    private ItemCompressedDiamondCharm()
    {
        super();
        this.setMaxStackSize(1);
        this.setCreativeTab(CreativeTab.TMod);

        ItemUtil.nameItemId(this, "compressed_diamond_charm");
    ...
    public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn)
    {
        ...
        tooltip.add("Gives Diamondskin.");
    }
    public static final Item item = new ItemCompressedDiamondCharm();

    @SubscribeEvent
    public static void registerItems(RegistryEvent.Register<Item> event) {
        event.getRegistry().register(ItemCompressedDiamondCharm.item);
    }
    ...
    public static Potion getPotion() {
        return PotionCoreEffects.POTIONS.get("diamond_skin");
    }
    ...
    public void updatePotionStatus(EntityLivingBase player, Potion potion) {
        ...
        if(playerIsWearingBauble(BaublesApi.getBaubles((EntityPlayer) player)))
            player.addPotionEffect(new PotionEffect(potion, Integer.MAX_VALUE, 2, true, false));
        else
